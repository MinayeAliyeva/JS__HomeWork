...
array.filter((item)=>item.age=="22"&&item.name.toLocaleLowerCase().startsWith("a")?console.log(item):null)
}
findAge(users)
//2.3.Return new array, if user is male add in front of name 'Mr', or Ms. (e.g. name:'Mr Anar', 'Ms Aynure').
function checkGender(array){
return array.map((item)=>item.gender==="female" ? `Mrs ${item.name}` : `Mr ${item.name}`)
}
...
function reversNums(num) {
  let operation = num.toString().split("").reverse().join("");
  return `${num}${operation}`;
}
console.log(reversNums(num1));
console.log(reversNums(num2));
console.log(reversNums(num3));
